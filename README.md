...
asymmetrically. I did not have a 1N34A diode on hand at the time, so in an attempt to emulate the reference clipping stage, my asymmetrical clipping configuration consists 
...
I used a potentiometer to find a good resistance value to properly bias the 2N3904 to ~4.5 V, since there are minor variations due to manufacturing differences between transistors. The value I
...
The silicon diodes only turn on at the very peaks of the wave. For most of the cycle, the diodes are completely off, allowing the transistor to switch very fast. The germanium and
...
Comparing the two FFTs, the second circuit is quieter, as the overall line begins ~10 dB lower than the first circuit. The second circuit is also more compressed, as the differences 
between the peaks and the noise floor is smaller than the first circuit. For instance, frequencies past the second and third harmonic in the second circuit vary significantly less 
than in the first. The second circuit also has a warmer tone due its faster frequency rolloff. For example, the first circuits moving average seems to cross the 70 dB threshold at 
around 6 kHz while the second circuit crosses the same point at roughly 2 kHz. As a result, higher frequencies are less noticable producing more warmth.
